...
def get_places(city_id, place_id):
    """Retrieves the list of all Place object if place_id is !none getPlace"""
...
def delete_place(place_id):
    """Deletes - Return: an empty dictionary with the status code 200"""
...
def create_place(request, city_id):
    """Creates a place object - Return: new place object"""
    check(City, city_id)
...
def search(request):
    """retrieves all Place objects depending of JSON - in body of request"""
    body_request = request.get_json()
    if body_request is None:
        abort(400, 'Not a JSON')
    places_list = []
...
def places_search():
    """retrieves all Place objects depending of the JSON in body of request"""
